...
# window, the same "pages blending together" bug class fixed this
# session for golf_client/financial_plumbing_client/portfolio_client/
# league_transactions_client/prediction_markets_client/
# market_internals/email_client/pages_conflicts (see each module's own
# comment) — directly relevant here since Weather's 7-day forecast was
# named in the very first live report of this bug. warm_cache below is
# wired into app.py's toast-check loop instead, unconditionally every
# rerun; both pages now read only the cached_* accessors, which never
