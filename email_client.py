...
# key than get_new_alerts' own FETCH_LOOKBACK_HOURS=6, so the toast
# loop's own warm 6h fetch never covered this page's 24h one) plus, for
# anything newly seen in that wider window, a real blocking Groq
# classification call. Live bug: this page blocking past app.py's 5s
# st_autorefresh window, corrupting the Streamlit rerun and blending
# pages together on screen. warm_daily_feed below is wired into
# app.py's toast-check loop instead (via fetch_throttle.run_bounded);
# pages_email.py now reads only cached_recent_important, which never
# fetches or classifies anything itself.
