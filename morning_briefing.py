...
def _ai_headline_and_body(facts_list: list[str], now: datetime) -> tuple[str, str] | None:
    ...
    prompt = (
        ...
        f"Today is {weekday} — a real, given fact, not a guess. The recent-days history above "
        f"intentionally shows several different real weekdays from OTHER, PAST days (each entry's "
        f"own parenthetical is THAT day's real weekday) — none of those is today. If you name a "
        f"day of the week anywhere in this response to describe what day it IS right now, it must "
        f"be exactly {weekday}, never one of the weekdays sitting in that history above, even "
        f"though those are also real and also correct for their own (different, past) date. Only "
        "worth mentioning today's own weekday at all if it actually connects to something below (a "
        ...
        "here. Session follow-up once this was first patched: don't let this rule scare you off "
        "dates entirely — a following test run went from stating a wrong date to naming NO date at "
        "all, even the completely real, already-given one for the approaching holiday above, and "
        "that overcorrection is its own real loss (\"Labour Day's coming up\" naming the actual real "
        "date is a genuinely good, wanted connection, confirmed live). The actual rule is narrower "
        "than \"avoid dates\": a specific calendar date (a month and day number, with or without a "
        "year) is completely fine, even encouraged when it earns a real connection, AS LONG AS that "
        "exact date already appears somewhere in the real data above or below (an upcoming holiday's "
        "own date, a calendar event's own date, anything already given) — copy it from there with "
        "full confidence. What's never allowed is reaching into your own general knowledge of "
        "calendars/history/any other year to invent one that isn't already sitting in the given data, "
        "however plausible it sounds. A weekday name alone (\"Wednesday\", \"this Saturday\") is fine "
        "under the rule just above regardless of this one.\n\n"
