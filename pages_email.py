"""Email page: every inbox email seen in the last WINDOW_SECONDS, flagged
for read/unread and AI-judged importance.
...
dedup state — that state only ever remembers what already toasted
(important emails), while this page's whole point, like the News page's
own, is to show everything that came through, not just what interrupted
you.
...
sight the way news.py's own "important" verdict deliberately is."""
...
def render():
    global _seen_at
    st.markdown('<div class="page-title page-title-email">Email — Last 24 Hours</div>', unsafe_allow_html=True)
    ...
    for mid, entry in entries:
        important = email_client.is_important(mid)
        # Falls back to the entry's own last-known state (there isn't
        # one to fall back to, so this only matters right at the
        # fetch-window boundary — see fetch_recent's shared 24h lookback
        # above, same window as WINDOW_SECONDS, so this is rare) rather
        # than a missing raw_by_id entry silently reading as "read."
        unread = raw_by_id[mid]["unread"] if mid in raw_by_id else False
        row_class = "news-feed-row-breaking" if important else ""
        unread_dot = '<span class="email-unread-dot"></span>' if unread else ""
        subject_class = "email-subject-unread" if unread else ""
        important_badge = '<span class="email-important-badge">Important</span>' if important else ""
        rows.append(
            f'<div class="news-feed-row {row_class}">'
            f'<div class="news-feed-headline">{unread_dot}'
            f'<span class="{subject_class}">{html.escape(entry["subject"])}</span>'
            f"{important_badge}</div>"
