...
def next_data_series() -> str | None:
    ...
    real Polymarket search per series (~1-2s each) on a cold cache,
    confirmed live at ~14s total combined. See warm_data_series_cache/
    cached_next_data_series below for why pages_predictions.py's
    render() no longer calls this directly."""
    candidates = []
...
def warm_data_series_cache() -> None:
    """Real (possibly ~14s cold) refresh, called from app.py's toast-
    check loop via fetch_throttle.run_bounded so pages_predictions.py's
    render() never pays this cost itself. next_data_series()'s own loop
    already populates _last_good_series for every series it touches as
    a side effect (see current_data_forecast) — this just also
    remembers which one was soonest, and only overwrites
    _last_good_next_series on a real hit so a transient all-series
...
def cached_data_forecast(series: str) -> dict | None:
    ...
    last real pass cached for `series`."""
    return _last_good_series.get(series)
