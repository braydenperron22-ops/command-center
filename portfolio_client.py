...
def _client() -> SnapTrade | None:
    ...
    # Full-audit finding, same class of bug as email_client.py's IMAP
    # connection: the SnapTrade SDK's own generated API methods (list_
    # user_accounts, get_account_balance_history, etc — confirmed via
    # inspect.signature on the installed package) accept no `timeout`
    # kwarg at all, and Configuration itself has no timeout field
    # either — every call through this client is genuinely UNBOUNDED
    # by default (urllib3's own PoolManager, with nothing overriding
    # it, has no ceiling). Confirmed live as a real hang: morning_
    # briefing.py's clause loop stalled on _portfolio_clause with no
    # exception ever raised, freezing the entire kiosk exactly like the
    # IMAP gap did. No supported public hook for this, so this reaches
    # into the pool manager's own connection_pool_kw — the same
    # dict urllib3 itself seeds every connection pool it opens from,
    # a well-established pattern for exactly this situation (an SDK
    # that never exposes a timeout knob), not a private/internal
    # implementation detail specific to this SDK.
    client.account_information.api_client.rest_client.pool_manager.connection_pool_kw["timeout"] = (
        urllib3.Timeout(connect=10, read=20)
    )
    return client
