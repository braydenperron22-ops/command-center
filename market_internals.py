...
# pages_internals.py's own render() was the sole caller of fear_greed_
# index/shiller_cape/price_ratio anywhere in the app — a cold cache
# (15min for the gauge, 6h for CAPE, whatever yfinance's own history
# TTL is for the ratios) meant render() itself paid the real fetch cost
# synchronously, risking blocking past app.py's 5s st_autorefresh
# window. warm_cache below is wired into app.py's toast-check loop
# instead (via fetch_throttle.run_bounded); pages_internals.py now
# reads only the cached_* accessors, which never fetch anything
# themselves.
